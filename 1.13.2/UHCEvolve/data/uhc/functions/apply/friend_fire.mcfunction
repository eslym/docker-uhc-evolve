#########################################################
# UHC Project 
# Name    : friend_fire.mcfunction 
# Made by : jelly99709
# Path    : /apply/
# Loop    : No. Called by uhc:go 
# func    : Set friendlyFire false
#########################################################

team modify red friendlyFire false
team modify blue friendlyFire false
team modify green friendlyFire false
team modify yellow friendlyFire false
team modify purple friendlyFire false
team modify gray friendlyFire false
team modify red_dark friendlyFire false
team modify blue_dark friendlyFire false
team modify green_dark friendlyFire false
team modify gold friendlyFire false
team modify pink friendlyFire false
team modify gray_dark friendlyFire false
team modify aqua friendlyFire false
team modify aqua_dark friendlyFire false